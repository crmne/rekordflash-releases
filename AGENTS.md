...
Keep the release notes source in sync with the workflow. If the workflow reads
a committed release-notes file, commit it before tagging. If the workflow
generates `release-notes.md`, update the configured source and publish that
file with `generate_release_notes: false`. Never leave GitHub's generated
notes in place.
